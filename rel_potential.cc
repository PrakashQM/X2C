                                ...
                                const GaussianShell& s2)
{
    ...
    int izm = 1;
    int iym = am1 + 1 + 1; // Prakash +1 added
    int ixm = iym * iym;
    int jzm = 1;
    int jym = am2 + 1+ 1;    // Prakash +1 added
    int jxm = jym * jym;
...
//      outfile->Printf("ixm= %d, jxm= %d\n", (am1+1)*(am1+1), (am2+1)*(am2+1));
  double     Ixx_1, Iyy_1, Izz_1, Itotal;
                    ...
                    for(int jj = 0; jj <= ii; jj++) {
                        ...
                        for(int kk = 0; kk <= am2; kk++) {
                            ...
                            for(int ll = 0; ll <= kk; ll++) {
                                ...
                                // <a+1|V|b+1> terms
                                Ixx_1 = 4.0*a1*a2*vi[iind+ixm][jind+jxm][0];
                                Iyy_1 = 4.0*a1*a2*vi[iind+iym][jind+jym][0];
                                Izz_1 = 4.0*a1*a2*vi[iind+izm][jind+jzm][0];
                               ...
                               // <a+1|V|b-1>, <a-1|V|b+1>, <a-1|V|b-1> terms
                                if (l1 && l2)
                                {
                                    Ixx_1 +=     l1*l2*vi[iind-ixm][jind-jxm][0];
#if RELVDEBUG
                                ...
                                if(l1)
                                {
                                    Ixx_1 -= 2.0*l1*a2*vi[iind-ixm][jind+jxm][0];
//                                    outfile->Printf("iind-ixm= %d, jind+jxm= %d \n", iind-ixm, jind+jxm);
                                }
                                if(l2)
                                {
                                    Ixx_1 -= 2.0*l2*a1*vi[iind+ixm][jind-jxm][0];
//                                    outfile->Printf("iind+ixm= %d, jind-jxm= %d \n", iind+ixm, jind-jxm);
                                }
                                if(m1 && m2)
                                {
                                    Iyy_1 +=     m1*m2*vi[iind-iym][jind-jym][0];
//                                    outfile->Printf("iind-iym= %d, jind-jym= %d \n", iind-iym, jind-jym);
                                }
                                if(m1)
                                {
                                    Iyy_1 -= 2.0*m1*a2*vi[iind-iym][jind+jym][0];
//                                    outfile->Printf("iind-iym= %d, jind+jym= %d \n", iind-iym, jind+jym);
                                }
                                if(m2)
                                {
                                    Iyy_1 -= 2.0*m2*a1*vi[iind+iym][jind-jym][0];
//                                    outfile->Printf("iind+iym= %d, jind-jym= %d \n", iind+iym, jind-jym);
                                }

                                if(n1 && n2)
                                {
                                    Izz_1 +=     n1*n2*vi[iind-izm][jind-jzm][0];
//                                    outfile->Printf("iind-izm= %d, jind-jzm= %d \n", iind-izm, jind-jzm);
                                }

                                if(n1)
                                {
                                    Izz_1 -= 2.0*n1*a2*vi[iind-izm][jind+jzm][0];
                                    //                               outfile->Printf("iind-izm= %d, jind+jzm= %d \n", iind-izm, jind+jzm);
                                }
                                if(n2)
                                {
                                    Izz_1 -= 2.0*n2*a1*vi[iind+izm][jind-jzm][0];
                                    //                               outfile->Printf("iind+izm= %d, jind-jzm= %d \n", iind+izm, jind-jzm);
                                }
                                Itotal = Ixx_1 + Iyy_1 + Izz_1;
